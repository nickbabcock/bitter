...
pub trait BitWriter {
    ...
    /// Write an arbitrary number of bits (up to 64) as unsigned value
    ///
    /// # Parameters
    ///
    ...
    /// Write an arbitrary number of bits (up to 64) as signed value
    ///
    ...
    ///
    /// * `bits` - Number of bits to write (0-64)
    /// * `value` - The signed value to write
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    /// Returns true if the writer is not partway through a byte
    ///
    /// This is useful for checking if you're at a byte boundary, which can be
    /// important for certain protocols or when you want to ensure byte alignment.
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    /// let mut writer = LittleEndianWriter::new(&mut buffer);
    ///
    /// assert!(writer.byte_aligned()); // Initially aligned
    ///
    /// writer.write_bit(true).unwrap();
    /// assert!(!writer.byte_aligned()); // Not aligned after writing 1 bit
    ///
    /// writer.write_bits(7, 0x7F).unwrap();
    /// assert!(writer.byte_aligned()); // Aligned again after completing the byte
    /// ```
